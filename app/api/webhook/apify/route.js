...
export async function POST(request) {
  try {
    ...
    // Exact matching fields mapped from the Apify Actor JSON
    ...
    const favoriteCount = productData.ux_layout_properties?.favorite_count || 0; // if available, or 0 since apify might not fetch it
    ...
    if (reviewsData.length > 0) {
      ...
      for (const review of reviewsData) {
        const safeAuthor = String(review.user_full_name || 'Anonim').substring(0, 250);
        let safeDate = '';
        if (review.created_at) {
          safeDate = new Date(review.created_at).toLocaleDateString('tr-TR');
        }
